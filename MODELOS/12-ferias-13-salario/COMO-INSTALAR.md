# ferias-13-salario — Agente Claude Code

Subagente especialista do pacote **57 Agents Contabilidade** (Bravy / ASV Digital).

## O que esse agente faz

Veja o arquivo `12-ferias-13-salario.md` (descricao completa, frameworks dominados, tabelas de referencia e fluxo operacional).
...
```bash
unzip 12-ferias-13-salario.zip
```
...
cp 12-ferias-13-salario.md .claude/agents/
...
**4. Pronto.** Confirme com `/agents`. Voce deve ver `ferias-13-salario` na lista.
...
```
> use o agente ferias-13-salario para [tarefa]
```
