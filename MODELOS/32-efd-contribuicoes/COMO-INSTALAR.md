# efd-contribuicoes — Agente Claude Code

Subagente especialista do pacote **57 Agents Contabilidade** (Bravy / ASV Digital).

## O que esse agente faz

Veja o arquivo `32-efd-contribuicoes.md` (descricao completa, frameworks dominados, tabelas de referencia e fluxo operacional).
...
```bash
unzip 32-efd-contribuicoes.zip
```
...
cp 32-efd-contribuicoes.md .claude/agents/
...
**4. Pronto.** Confirme com `/agents`. Voce deve ver `efd-contribuicoes` na lista.
...
```
> use o agente efd-contribuicoes para [tarefa]
```
