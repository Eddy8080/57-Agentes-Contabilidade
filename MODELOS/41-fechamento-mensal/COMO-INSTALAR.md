# fechamento-mensal — Agente Claude Code

Subagente especialista do pacote **57 Agents Contabilidade** (Bravy / ASV Digital).

## O que esse agente faz

Veja o arquivo `41-fechamento-mensal.md` (descricao completa, frameworks dominados, tabelas de referencia e fluxo operacional).
...
```bash
unzip 41-fechamento-mensal.zip
```
...
cp 41-fechamento-mensal.md .claude/agents/
...
**4. Pronto.** Confirme com `/agents`. Voce deve ver `fechamento-mensal` na lista.
...
```
> use o agente fechamento-mensal para [tarefa]
```
