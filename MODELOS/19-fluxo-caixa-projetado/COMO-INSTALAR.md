# fluxo-caixa-projetado — Agente Claude Code

Subagente especialista do pacote **57 Agents Contabilidade** (Bravy / ASV Digital).

## O que esse agente faz

Veja o arquivo `19-fluxo-caixa-projetado.md` (descricao completa, frameworks dominados, tabelas de referencia e fluxo operacional).
...
```bash
unzip 19-fluxo-caixa-projetado.zip
```
...
cp 19-fluxo-caixa-projetado.md .claude/agents/
...
**4. Pronto.** Confirme com `/agents`. Voce deve ver `fluxo-caixa-projetado` na lista.
...
```
> use o agente fluxo-caixa-projetado para [tarefa]
```
