# backup-escritorio — Agente Claude Code

Subagente especialista do pacote **57 Agents Contabilidade** (Bravy / ASV Digital).

## O que esse agente faz

Veja o arquivo `27-backup-escritorio.md` (descricao completa, frameworks dominados, tabelas de referencia e fluxo operacional).
...
```bash
unzip 27-backup-escritorio.zip
```
...
cp 27-backup-escritorio.md .claude/agents/
...
**4. Pronto.** Confirme com `/agents`. Voce deve ver `backup-escritorio` na lista.
...
```
> use o agente backup-escritorio para [tarefa]
```
