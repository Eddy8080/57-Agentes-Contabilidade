# calculo-irrf-folha — Agente Claude Code

Subagente especialista do pacote **57 Agents Contabilidade** (Bravy / ASV Digital).

## O que esse agente faz

Veja o arquivo `30-calculo-irrf-folha.md` (descricao completa, frameworks dominados, tabelas de referencia e fluxo operacional).
...
```bash
unzip 30-calculo-irrf-folha.zip
```
...
cp 30-calculo-irrf-folha.md .claude/agents/
...
**4. Pronto.** Confirme com `/agents`. Voce deve ver `calculo-irrf-folha` na lista.
...
```
> use o agente calculo-irrf-folha para [tarefa]
```
