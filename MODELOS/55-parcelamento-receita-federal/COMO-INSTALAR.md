# parcelamento-receita-federal — Agente Claude Code

Subagente especialista do pacote **57 Agents Contabilidade** (Bravy / ASV Digital).

## O que esse agente faz

Veja o arquivo `55-parcelamento-receita-federal.md` (descricao completa, frameworks dominados, tabelas de referencia e fluxo operacional).
...
```bash
unzip 55-parcelamento-receita-federal.zip
```
...
cp 55-parcelamento-receita-federal.md .claude/agents/
...
**4. Pronto.** Confirme com `/agents`. Voce deve ver `parcelamento-receita-federal` na lista.
...
```
> use o agente parcelamento-receita-federal para [tarefa]
```
