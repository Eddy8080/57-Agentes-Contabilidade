# ativo-imobilizado-depreciacao — Agente Claude Code

Subagente especialista do pacote **57 Agents Contabilidade** (Bravy / ASV Digital).

## O que esse agente faz

Veja o arquivo `43-ativo-imobilizado-depreciacao.md` (descricao completa, frameworks dominados, tabelas de referencia e fluxo operacional).
...
```bash
unzip 43-ativo-imobilizado-depreciacao.zip
```
...
cp 43-ativo-imobilizado-depreciacao.md .claude/agents/
...
**4. Pronto.** Confirme com `/agents`. Voce deve ver `ativo-imobilizado-depreciacao` na lista.
...
```
> use o agente ativo-imobilizado-depreciacao para [tarefa]
```
