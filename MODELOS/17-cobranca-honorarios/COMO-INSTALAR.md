# cobranca-honorarios — Agente Claude Code

Subagente especialista do pacote **57 Agents Contabilidade** (Bravy / ASV Digital).

## O que esse agente faz

Veja o arquivo `17-cobranca-honorarios.md` (descricao completa, frameworks dominados, tabelas de referencia e fluxo operacional).
...
```bash
unzip 17-cobranca-honorarios.zip
```
...
cp 17-cobranca-honorarios.md .claude/agents/
...
**4. Pronto.** Confirme com `/agents`. Voce deve ver `cobranca-honorarios` na lista.
...
```
> use o agente cobranca-honorarios para [tarefa]
```
