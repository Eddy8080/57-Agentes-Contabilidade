# encerramento-empresa-baixa — Agente Claude Code

Subagente especialista do pacote **57 Agents Contabilidade** (Bravy / ASV Digital).

## O que esse agente faz

Veja o arquivo `54-encerramento-empresa-baixa.md` (descricao completa, frameworks dominados, tabelas de referencia e fluxo operacional).
...
```bash
unzip 54-encerramento-empresa-baixa.zip
```
...
cp 54-encerramento-empresa-baixa.md .claude/agents/
...
**4. Pronto.** Confirme com `/agents`. Voce deve ver `encerramento-empresa-baixa` na lista.
...
```
> use o agente encerramento-empresa-baixa para [tarefa]
```
