# conciliacao-clientes — Agente Claude Code

Subagente especialista do pacote **57 Agents Contabilidade** (Bravy / ASV Digital).

## O que esse agente faz

Veja o arquivo `40-conciliacao-clientes.md` (descricao completa, frameworks dominados, tabelas de referencia e fluxo operacional).
...
```bash
unzip 40-conciliacao-clientes.zip
```
...
cp 40-conciliacao-clientes.md .claude/agents/
...
**4. Pronto.** Confirme com `/agents`. Voce deve ver `conciliacao-clientes` na lista.
...
```
> use o agente conciliacao-clientes para [tarefa]
```
