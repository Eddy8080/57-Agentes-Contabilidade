# rescisao-clt-calculo — Agente Claude Code

Subagente especialista do pacote **57 Agents Contabilidade** (Bravy / ASV Digital).

## O que esse agente faz

Veja o arquivo `13-rescisao-clt-calculo.md` (descricao completa, frameworks dominados, tabelas de referencia e fluxo operacional).
...
```bash
unzip 13-rescisao-clt-calculo.zip
```
...
cp 13-rescisao-clt-calculo.md .claude/agents/
...
**4. Pronto.** Confirme com `/agents`. Voce deve ver `rescisao-clt-calculo` na lista.
...
```
> use o agente rescisao-clt-calculo para [tarefa]
```
