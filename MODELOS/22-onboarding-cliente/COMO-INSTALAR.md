# onboarding-cliente — Agente Claude Code

Subagente especialista do pacote **57 Agents Contabilidade** (Bravy / ASV Digital).

## O que esse agente faz

Veja o arquivo `22-onboarding-cliente.md` (descricao completa, frameworks dominados, tabelas de referencia e fluxo operacional).
...
```bash
unzip 22-onboarding-cliente.zip
```
...
cp 22-onboarding-cliente.md .claude/agents/
...
**4. Pronto.** Confirme com `/agents`. Voce deve ver `onboarding-cliente` na lista.
...
```
> use o agente onboarding-cliente para [tarefa]
```
