# documentos-pendentes — Agente Claude Code

Subagente especialista do pacote **57 Agents Contabilidade** (Bravy / ASV Digital).

## O que esse agente faz

Veja o arquivo `21-documentos-pendentes.md` (descricao completa, frameworks dominados, tabelas de referencia e fluxo operacional).
...
```bash
unzip 21-documentos-pendentes.zip
```
...
cp 21-documentos-pendentes.md .claude/agents/
...
**4. Pronto.** Confirme com `/agents`. Voce deve ver `documentos-pendentes` na lista.
...
```
> use o agente documentos-pendentes para [tarefa]
```
