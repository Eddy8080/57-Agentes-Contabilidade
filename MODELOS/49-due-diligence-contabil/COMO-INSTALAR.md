# due-diligence-contabil — Agente Claude Code

Subagente especialista do pacote **57 Agents Contabilidade** (Bravy / ASV Digital).

## O que esse agente faz

Veja o arquivo `49-due-diligence-contabil.md` (descricao completa, frameworks dominados, tabelas de referencia e fluxo operacional).
...
```bash
unzip 49-due-diligence-contabil.zip
```
...
cp 49-due-diligence-contabil.md .claude/agents/
...
**4. Pronto.** Confirme com `/agents`. Voce deve ver `due-diligence-contabil` na lista.
...
```
> use o agente due-diligence-contabil para [tarefa]
```
