# follow-up-cliente — Agente Claude Code

Subagente especialista do pacote **57 Agents Contabilidade** (Bravy / ASV Digital).

## O que esse agente faz

Veja o arquivo `23-follow-up-cliente.md` (descricao completa, frameworks dominados, tabelas de referencia e fluxo operacional).
...
```bash
unzip 23-follow-up-cliente.zip
```
...
cp 23-follow-up-cliente.md .claude/agents/
...
**4. Pronto.** Confirme com `/agents`. Voce deve ver `follow-up-cliente` na lista.
...
```
> use o agente follow-up-cliente para [tarefa]
```
